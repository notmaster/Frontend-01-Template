  ...
  for (let standard of standards) {
    ...
    try {
      ...
      const elements = Array.from(
        iframe.contentWindow.document.querySelectorAll('a.property')
      )
      console.log(elements)
      if (elements.length)
        propertiesElements = propertiesElements.concat(elements)
    } catch (err) {
      console.log(err)
    }
  }
  property.forEach((item) => {
    const standard = propertiesElements.find(
      (item2) => item2.innerText.toLowerCase() === item.toLowerCase()
    )
    if (standard) {
